...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (typeof m.text !== 'string')
            m.text = ''
        const premis = global.db.data.users.premium
        ...
        const isBotAdmin = isMods || bot?.admin|| false // Are you Admin?
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user now he is administrator')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.imgur.com/whjlJSf.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user no longer administrator')
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '📇 Register to use this function Typing:\n\n*/reg name.age*\n\n📌Example : */reg Zero.16*',
