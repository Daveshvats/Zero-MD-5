...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '*[❗𝐈𝐍𝐅𝐎❗] HOW TO USE THIS COMMAND?*\n—◉ _#logo (effect) (text)_\n*EXAMPLE:*\n—◉ _#logo 3d-deep-sea-metal Zero_\n\n*[❗] WHEN I TELL THEM THAT A TEXT IS MISSING THE USE WOULD BE:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*EXAMPLE:*\n—◉ _#logo Wolf-Logo-Galaxy Zero|Bot_\n\n*<EFFECTS LIST/>*\n\n° ඬ⃟📝 #logo ' + effects.map((v) => v.title).join('\n° ඬ⃟📝 #logo ');
  effect = effect.toLowerCase();
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗𝐈𝐍𝐅𝐎❗] THE EFFECT ${effect} IT IS NOT IN THE LIST OF EFFECTS*`;
  let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart();
  if (text.includes(split)) text = text.split(split);
  text = Array.isArray(text) ? text : [text];
  const res = await textpro(effect, ...text);
  if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] EFFECT ${effect} IS NOT IN THE LIST OF EFFECTS*` : `*[❗𝐈𝐍𝐅𝐎❗] THE CORRECT USE OF THE COMMAND IS ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
